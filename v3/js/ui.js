...
// ── 네비게이션 ─────────────────────────────────────────
const PAGE_MAP = { home:‘page-home’, forecast:‘page-forecast’, entries:‘page-entries’, cards:‘page-cards’, ledger:‘page-ledger’, settings:‘page-settings’ };
...
if (btn) btn.textContent = state.theme===‘light’ ? ‘라이트 모드’ : ‘다크 모드’;
}
...
save(); renderModule.renderHome();
if (document.getElementById(‘page-forecast’).classList.contains(‘active’)) renderModule.renderForecast();
...
if (avatar) {
avatar.innerHTML = user?.photoURL ? `<img src="${user.photoURL}" referrerpolicy="no-referrer">` : (user?.displayName||user?.email||’?’)[0].toUpperCase();
}
openSheet(‘profile-sheet’);
}

export function confirmSignOut() { closeSheet(‘profile-sheet’); openSheet(‘signout-sheet’); }
...
if (key===‘dangerLine’) { state.dangerLine=parseFloat(val)||0; document.getElementById(‘danger-line-input’).value=state.dangerLine; save(); renderModule.renderHome(); }
}
...
state.editId=entry?entry.id:null; state.formType=entry?entry.type:‘income’;
...
export function hideForm() { document.getElementById(‘form-overlay’).style.display=‘none’; document.body.style.overflow=’’; state.editId=null; }
export function closeFormIfOutside(e) { if (e.target===document.getElementById(‘form-overlay’)) hideForm(); }
export function setFormType(t) { state.formType=t; _updateTypeButtons(t); _updateCatOptions(t); }
...
document.getElementById(‘f-category’).innerHTML=cats.map(c=>`<option value="${c}" ${c===selected?'selected':''}>${c}</option>`).join(’’);
}
export function onRepeatChange() {
const rep=document.getElementById(‘f-repeat’).value;
document.getElementById(‘day-label’).textContent=rep===‘1회성’?‘기준일’:rep===‘격주’?‘시작일’:‘기준일 (매월 며칠)’;
...
const entry = { id:state.editId||uid(), type:state.formType, name, amount, category:document.getElementById(‘f-category’).value, repeat:document.getElementById(‘f-repeat’).value, day:parseInt(document.getElementById(‘f-day’).value)||null, card:document.getElementById(‘f-card’).value, endMonth:document.getElementById(‘f-endmonth’).value.trim()||null, date:document.getElementById(‘f-date’).value||null };
if (state.editId) state.entries=state.entries.map(e=>e.id===state.editId?entry:e);
else state.entries.push(entry);
hideForm(); save(); renderModule.renderAll();
showBadge(‘✅ 저장됨’);
}

export function editEntry(id) { const e=state.entries.find(x=>x.id===id); if(e) showForm(e); }
export function deleteEntry(id) { if(!confirm(‘삭제할까요?’)) return; state.entries=state.entries.filter(e=>e.id!==id); save(); renderModule.renderAll(); }
...
export function closeLedgerEditor() { document.getElementById(‘ledger-editor-card’).style.display=‘none’; }

export function saveLedgerExpense() {
if (!_selectedLedgerDate) return;
const amt=parseFloat(document.getElementById(‘ledger-amount-input’).value)||0;
if (!state.checkData) state.checkData={};
if (amt>0) state.checkData[_selectedLedgerDate]=amt; else delete state.checkData[_selectedLedgerDate];
syncCheckDataToBalance(); save(); renderModule.renderLedger(); renderModule.renderHome();
if (document.getElementById(‘page-forecast’).classList.contains(‘active’)) renderModule.renderForecast();
showBadge(‘✅ 가계부 저장됨’);
}
...
if (!_selectedLedgerDate) return;
if (state.checkData) delete state.checkData[_selectedLedgerDate];
syncCheckDataToBalance(); save();
document.getElementById(‘ledger-amount-input’).value=’’;
renderModule.renderLedger(); renderModule.renderHome();
showBadge(‘🗑️ 가계부 삭제됨’);
}
...
a.download=`cashflow_${dateKey(today())}.json`; a.click();
}
export function importDataClick() { document.getElementById(‘import-file’).click(); }
export function importData(e) {
const file=e.target.files[0]; if(!file) return;
new FileReader().onload=ev=>{ try{ Object.assign(state,JSON.parse(ev.target.result)); save(); renderModule.renderAll(); showBadge(‘✅ 가져오기 완료’); } catch{ alert(‘파일 형식 오류’); } };
const r=new FileReader(); r.onload=ev=>{ try{ Object.assign(state,JSON.parse(ev.target.result)); save(); renderModule.renderAll(); showBadge(‘✅ 가져오기 완료’); } catch{ alert(‘파일 형식 오류’); } }; r.readAsText(file);
}
export function resetAll() { state.entries=[]; state.cardData={}; state.checkData={}; state.appliedCheckData={}; state.balance=0; save(); applyTheme(); renderModule.renderAll(); }
