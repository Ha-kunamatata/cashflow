...
// ════════════════════════════════════════════════════════
import { initAuth, signInWithGoogle, signOutUser, loadFromFirebase, startSync } from ‘./firebase.js’;
...
setChartPeriod, setForecastFilter, setEntryFilter, changeLedgerMonth } from ‘./render.js’;
...
editEntry, deleteEntry, updateCardData,
openLedgerEditor, closeLedgerEditor,
saveLedgerExpense, clearLedgerExpense,
saveSetting, onDangerLineChange,
exportData, importDataClick, importData, resetAll } from ‘./ui.js’;
import { initRipple, showBadge, openSheet, closeSheet, closeSheetOutside, showLoading, hideLoading } from ‘./utils.js’;
...
else { txt.textContent=(user.displayName||user.email||'?')[0].toUpperCase(); img.style.display='none'; txt.style.display='flex'; }
...
if (cloud) { Object.assign(state, cloud); localStorage.setItem('cashflow_v21', JSON.stringify(state)); showBadge('☁️ 동기화됨'); }
initDefaultData();
...
document.querySelectorAll(’[data-forecast-filter]’).forEach(btn => { btn.addEventListener(‘click’, () => setForecastFilter(btn.dataset.forecastFilter, btn)); });
...
document.querySelectorAll(’[data-entry-filter]’).forEach(btn => { btn.addEventListener(‘click’, () => setEntryFilter(btn.dataset.entryFilter, btn)); });
...
document.getElementById(‘btn-signout-confirm’)?.addEventListener(‘click’, async () => { closeSheet(‘signout-sheet’); showLoading(‘로그아웃 중…’); await signOutUser(); hideLoading(); showBadge(‘👋 로그아웃되었습니다’); });
