...
export let selectedLedgerDate = null;
...
const mh = state.entries.filter(e =>
e.type===‘expense’ && e.category===‘할부’ && e.repeat===‘매월’ &&
(!e.endMonth || parseInt(e.endMonth) >= todayYm)
).reduce((s,e)=>s+e.amount, 0);

document.getElementById(‘sum-income’).textContent  = fmtFull(mi);
document.getElementById(‘sum-expense’).textContent = fmtFull(me);
document.getElementById(‘sum-halbu’).textContent   = fmtFull(mh);

const monthPrefix = `${today().getFullYear()}-${p2(today().getMonth()+1)}`;
const checkTotal  = Object.entries(state.checkData||{})
.filter(([d]) => d.startsWith(monthPrefix))
.reduce((s,[,v]) => s+v, 0);
const checkEl = document.getElementById(‘sum-checkcard’);
if (checkEl) checkEl.textContent = fmtFull(checkTotal);
...
if (insightEl) {
insightEl.textContent = state.balance < state.dangerLine
? ‘주의가 필요해요. 현재 잔고가 위험 기준선 아래로 내려갈 수 있어요.’
: net > 0
...
document.getElementById(‘balance-warning-text’).textContent =
`${firstDanger.date.getMonth()+1}/${firstDanger.date.getDate()} 이후 잔고 위험 예상`;
} else {
...
const list = dangerDays.slice(0,3).map(d=>`${d.date.getMonth()+1}/${d.date.getDate()}(${fmtShort(d.balance)})`).join(’, ’);
...
export function setForecastFilter(f, btn) {
_forecastFilter = f;
document.querySelectorAll(’#page-forecast .filter-tab’).forEach(b=>b.classList.remove(‘active’));
btn.classList.add(‘active’);
renderForecastTable();
}
...
let labels=’’;
const step = _chartPeriod<=30?7:*chartPeriod<=90?14:30;
fc.filter((*,i)=>i%step===0).forEach((f,idx)=>{
labels+=`<text x="${(idx*step*bw+bw/2).toFixed(1)}" y="${H+2}" fill="#475569" font-size="9" font-family="monospace" text-anchor="middle">${f.date.getMonth()+1}/${f.date.getDate()}</text>`;
});

document.getElementById(‘forecast-chart’).innerHTML = ` <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"> <stop offset="0%" stop-color="#3b82f6" stop-opacity="0.35"/> <stop offset="100%" stop-color="#3b82f6" stop-opacity="0.03"/> </linearGradient></defs> ${state.dangerLine>min?`<rect x="0" y="${dangerY.toFixed(1)}" width="${W}" height="${(zeroY-dangerY).toFixed(1)}" fill="rgba(249,115,22,0.06)"/>`:''} <line x1="0" y1="${zeroY.toFixed(1)}" x2="${W}" y2="${zeroY.toFixed(1)}" stroke="#334155" stroke-width="1" stroke-dasharray="4 4"/> ${state.dangerLine>0?`<line x1="0" y1="${dangerY.toFixed(1)}" x2="${W}" y2="${dangerY.toFixed(1)}" stroke="#f97316" stroke-width="1.5" stroke-dasharray="4 3"/>
<text x="4" y="${(dangerY-4).toFixed(1)}" fill="#f97316" font-size="8" font-family="monospace">${fmtShort(state.dangerLine)}</text>`:''} <polygon points="0,${H} ${pts} ${W},${H}" fill="url(#ag)"/> <polyline points="${pts}" fill="none" stroke="#3b82f6" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/> ${dots}${labels}`;
}
...
const fixedIncome = state.entries.filter(e=>e.type===‘income’&&e.repeat===‘매월’&&(!e.endMonth||parseInt(e.endMonth)>=ym)).reduce((s,e)=>s+e.amount,0);
const fixedExpense= state.entries.filter(e=>e.type===‘expense’&&e.repeat===‘매월’&&(!e.endMonth||parseInt(e.endMonth)>=ym)).reduce((s,e)=>s+e.amount,0);
const cd = state.cardData[ymStr]||{};
months.push({ label:`${d.getMonth()+1}월`, income:fixedIncome, expense:fixedExpense+(cd.hyundai||0)+(cd.kookmin||0) });
}
const maxVal = Math.max(…months.map(m=>Math.max(m.income,m.expense)), 1);
document.getElementById(‘monthly-chart’).innerHTML = ` <div class="monthly-chart-bar"> ${months.map(m=>`
<div>
<div style="font-size:9px;color:var(--text3);margin-bottom:4px;font-weight:600">${m.label}</div>
<div class="bar-row"><span class="bar-label" style="color:var(--green2)">수입</span><div class="bar-track"><div class="bar-fill income" style="width:${(m.income/maxVal*100).toFixed(1)}%"></div></div><span class="bar-value" style="color:var(--green2)">${fmtShort(m.income)}</span></div>
<div class="bar-row"><span class="bar-label" style="color:var(--red2)">지출</span><div class="bar-track"><div class="bar-fill expense" style="width:${(m.expense/maxVal*100).toFixed(1)}%"></div></div><span class="bar-value" style="color:var(--red2)">${fmtShort(m.expense)}</span></div>
</div>`).join('<div style="height:8px"></div>')} </div>`;
}
...
if (m!==lastMonth) { html+=`<tr class="month-header"><td colspan="6">${f.date.getFullYear()}년 ${m}월</td></tr>`; lastMonth=m; }
const isDanger = f.balance<state.dangerLine;
const dow      = f.date.getDay();
const dayClass = ‘col-day’+(dow===0?’ sun’:dow===6?’ sat’:’’);
const isToday  = dateKey(f.date)===dateKey(today());
html+=`<tr class="${isDanger?'danger':f.income>0?'income-row':''}"> <td class="col-date" style="${isToday?'font-weight:700;color:var(--accent2)':''}">${m}/${p2(f.date.getDate())}</td> <td class="${dayClass}">${DAYS_KR[dow]}</td> <td class="col-in">${f.income>0?fmtShort(f.income):'-'}</td> <td class="col-out">${f.expense>0?fmtShort(f.expense):'-'}</td> <td class="col-bal${isDanger?' danger':''}">${fmtShort(f.balance)}</td> <td class="col-status">${isDanger?'⚠️':f.income&&f.expense?'💰💳':f.income?'💰':f.expense?'💳':'-'}</td> </tr>`;
}
...
// 수입/지출 목록
// ════════════════════════════════════════════════════════
export function setEntryFilter(f, btn) {
_entryFilter = f;
document.querySelectorAll(’#page-entries .filter-tab’).forEach(b=>b.classList.remove(‘active’));
btn.classList.add(‘active’);
renderEntries();
}
...
document.getElementById(‘ledger-month-label’).textContent = `${y}년 ${m+1}월`;
...
const avgSpend     = spendDays>0 ? Math.round(monthTotal/spendDays) : 0;
let maxDayText     = ‘-’;
...
const totalEl = document.getElementById(‘ledger-total-spend’);
const avgEl   = document.getElementById(‘ledger-avg-spend’);
const maxEl   = document.getElementById(‘ledger-max-day’);
if (totalEl) totalEl.textContent = fmtShort(monthTotal);
if (avgEl)   avgEl.textContent   = spendDays>0 ? fmtShort(avgSpend) : ‘-’;
if (maxEl)   maxEl.textContent   = maxDayText;

document.getElementById(‘ledger-month-summary’).innerHTML = ` <div class="ledger-summary-hero"> <div><strong>${m+1}월 소비 요약</strong><span>날짜를 눌러 금액을 입력하거나 수정할 수 있어요</span></div> <div style="font-family:var(--mono);font-size:16px;color:var(--orange);font-weight:800">${fmtShort(monthTotal)}</div> </div>`;
...
const isSelected = dk===selectedLedgerDate;
const amt = (state.checkData&&state.checkData[dk]) ? state.checkData[dk] : 0;
let numClass = ‘ledger-day-num’+(dow===0?’ sun’:dow===6?’ sat’:’’);
html+=`<div class="ledger-day ${isToday?'today':''} ${isSelected?'selected':''}" onclick="window._ui.openLedgerEditor('${dk}')"> <div class="ledger-day-top"><div class="${numClass}">${day}</div></div> <div class="ledger-day-amount">${amt>0?'-'+fmtShort(amt):''}</div> </div>`;
}
document.getElementById(‘ledger-calendar-grid’).innerHTML = html;
}
